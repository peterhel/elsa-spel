...
class Hero {
  ...
  render(ctx, env) {
    ...
    if (movementChange[1] !== 0) { //If moving up or down
      let pm = movementChange[1] / Math.abs(movementChange[1]);

      let feetZone = arrayToInt(ctx.getImageData(pxHeroCenter, pxHeroBottom + 1 * pm, 1, 1).data)
    ...
    let [r1,atFeet,,,r2,belowFeet] = ctx.getImageData(pxHeroCenter, pxHeroBottom-1, 1, 2).data
    ...
    //   console.log("falling");
      j++;
      let [,atFeet,,,,belowFeet] = ctx.getImageData(pxHeroCenter, pxHeroBottom-1+j, 1, 2).data
